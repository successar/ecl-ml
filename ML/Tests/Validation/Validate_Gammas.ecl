// Check incomplete gamma functions against Python scipy implementation
// numpy and scipy must be installed to use this attribute
IMPORT ML.Utils;
IMPORT Python;
...
Work1 makeT(UNSIGNED ts) := TRANSFORM
  ...
  SELF.x := x;
  SELF.z := z;
  SELF.g := Utils.gamma(x);
  SELF.l := Utils.lowerGamma(x, z);
  SELF.u := Utils.upperGamma(x, z);
  SELF.py_g := scipy_gamma(x);
  SELF.py_l := scipy_lowerGamma(x,z);
  SELF.py_u := scipy_upperGamma(x,z);
  g_denom := MAX(scipy_gamma(x), Utils.gamma(x));
  g_abs_diff := ABS(scipy_gamma(x) - Utils.gamma(x));
  SELF.g_diff := IF(g_abs_diff/g_denom < epsilon, 0, g_abs_diff);
  l_denom := MAX(scipy_lowerGamma(x, z), Utils.lowerGamma(x, z));
  l_abs_diff := ABS(scipy_lowerGamma(x, z) - Utils.lowerGamma(x, z));
  SELF.l_diff := IF(l_abs_diff/l_denom < epsilon, 0, l_abs_diff);
  u_denom := MAX(scipy_upperGamma(x, z), Utils.upperGamma(x, z));
  u_abs_diff := ABS(scipy_upperGamma(x, z) - Utils.upperGamma(x, z));
